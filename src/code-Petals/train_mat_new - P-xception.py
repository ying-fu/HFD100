...
if __name__ == "__main__":
    ...
    print("Start Training, xception!")  # 定义遍历数据集的次数
    with open("results/acc_31xception_P100_HSI.txt", "w") as f:
        with open("results/log31P100xc.txt", "w")as f2:
            for epoch in range(pre_epoch, EPOCH):
                ...
                with torch.no_grad():
                    ...
                    if acc > best_acc:
                        f3 = open("results/best_acc_31xception_P100_HSI.txt", "w")
                        f3.write("EPOCH=%d,best_acc= %.3f%%" % (epoch + 1, acc))
                        f3.close()
                        best_acc = acc
                        torch.save(net, 'results/model31_xception_P100_HSI_model_best.pkl')
