...
def load_split_train_test():
    ...
    train = h5py.File('data/petals100/train/train31.h5', 'r')
    train.keys()
    hsi = train['HSI'][:]
    label = train['label'][:]
    train_data = Dataset_HSI(hsi, label)
    test = h5py.File('data/petals100/test/test31.h5', 'r')
    test.keys()
    test_hsi = test['HSI'][:]
...
# 模型定义-ResNet
net = EfficientNetB0(31,100).to(device)
# 定义损失函数和优化方式
...
if __name__ == "__main__":
    ...
    with open("results/acc_31efficientnet_P100_HSI.txt", "w") as f:
        with open("results/log31P100ef.txt", "w")as f2:
            for epoch in range(pre_epoch, EPOCH):
                ...
                with torch.no_grad():
                    ...
                    if acc > best_acc:
                        f3 = open("results/best_acc_31efficientnet_P100_HSI.txt", "w")
                        f3.write("EPOCH=%d,best_acc= %.3f%%" % (epoch + 1, acc))
                        f3.close()
                        best_acc = acc
                        torch.save(net, 'results/model31_efficientnet_P100_HSI_model_best.pkl')
