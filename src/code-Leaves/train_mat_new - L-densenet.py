...
# 模型定义-ResNet
net = densenet121(31,60).to(device)
# 定义损失函数和优化方式
...
if __name__ == "__main__":
    ...
    with open("results/acc_31densenet_L60_HSI.txt", "w") as f:
        with open("results/log31L60de.txt", "w")as f2:
            for epoch in range(pre_epoch, EPOCH):
                ...
                with torch.no_grad():
                    ...
                    if acc > best_acc:
                        f3 = open("results/best_acc_31densenet_L60_HSI.txt", "w")
                        f3.write("EPOCH=%d,best_acc= %.3f%%" % (epoch + 1, acc))
                        f3.close()
                        best_acc = acc
                        torch.save(net, 'results/model31_densenet_L60_HSI_model_best.pkl')
